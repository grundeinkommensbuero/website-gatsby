...
const getRate = ({ current, previous, comparisonAttribute, interval }) => {
  return (
    ((current[comparisonAttribute] - previous[comparisonAttribute]) /
...
export const Ticker = ({ tickerDescription }) => {
  const { municipality, allMunicipalityStats } = useContext(
    MunicipalityContext
  );
  const [peopleCount, setPeopleCount] = useState(3592);
  const [municipalityCount, setMunicipalityCount] = useState(43);

  console.log(allMunicipalityStats);

  const { summary } = allMunicipalityStats;
  const { users, timestamp, previous } = summary;
  const current = { users, timestamp };

  console.log(
    getRate({
      current,
      previous,
      comparisonAttribute: 'users',
      interval: 1000 * 60,
    })
  );
  ...
  useEffect(() => {
    ...
    // console.log(
    //   'firePeopleCounter: ',
    //   statsSummary && statsSummary.users > statsSummary.previous.users
    // );
    ...
    // console.log(
    //   'fireMunicipalityCounter: ',
    //   statsSummary && statsSummary.users > statsSummary.previous.users
    // );

    if (
      statsSummary &&
      statsSummary.municipalities > statsSummary.previous.municipalities
    ) {
